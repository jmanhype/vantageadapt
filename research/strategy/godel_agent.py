...
class GodelAgent:
    ...
    def track_performance(self, metrics: Dict[str, Any], parameters: Dict[str, Any], 
        ...
        if not self.last_metrics:
            ...
            # Check if improvement threshold was met
            improvement = (current_score - last_score) / max(0.1, abs(last_score))
    ...
    def get_successful_parameters(self, market_regime: str) -> Optional[Dict[str, Any]]:
        ...
        if market_regime not in self.success_patterns or not self.success_patterns[market_regime]:
            return None
            
        # Return parameters from the highest scoring run
        successful_params = self.success_patterns[market_regime]
        return successful_params[0]['parameters']  # Best performing parameters
