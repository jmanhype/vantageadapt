...
class Backtester:
    ...
    def forward(self, trade_data: Dict[str, pd.DataFrame], parameters: Dict[str, Any], conditions: Dict[str, List[str]]) -> Dict[str, Any]:
        ...
        try:
            ...
            max_drawdown = results['backtest_results'].get('metrics', {}).get('trade_memory_stats', {}).get('max_drawdown', 0)
            
            validation_passed = (
                total_return >= self.performance_thresholds.get('min_return', 0.10) and
                total_trades >= self.performance_thresholds.get('min_trades', 10) and
                abs(max_drawdown) <= self.performance_thresholds.get('max_drawdown', 0.20)
            )
            
            results['validation_passed'] = validation_passed
            results['performance_thresholds'] = self.performance_thresholds
